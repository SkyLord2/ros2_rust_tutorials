...
private:
    void on_timer()
    {
        ...
        try {
            // 获取ROS系统的当前时间 / 监听当前时刻源坐标系到目标坐标系的坐标变换[cite: 13]
        ...
        // 如果坐标变换获取失败，进入异常报告[cite: 13]
        catch (const tf2::TransformException & ex) {
            RCLCPP_INFO(
                this->get_logger(),
                "Could not transform %s to %s: %s",
                target_frame_.c_str(), source_frame_.c_str(), ex.what()); //[cite: 13]
            return; //[cite: 13]
        }
        ...
        // 3. 从旋转矩阵中提取欧拉角 (对应 Python 的 euler_from_quaternion)[cite: 13]
        m.getRPY(roll, pitch, yaw);

        // 打印信息：输出原点与目标的位移和姿态[cite: 13]
        RCLCPP_INFO(
            this->get_logger(),
            "Get %s --> %s transform: [%f, %f, %f] [%f, %f, %f]",
            source_frame_.c_str(), target_frame_.c_str(),
            pos.x, pos.y, pos.z,
            roll, pitch, yaw); //[cite: 13]
    }
public:
    explicit TFListener(std::string name) : Node(name)
    {
        ...
        this->declare_parameter<std::string>("target_frame", "house");
