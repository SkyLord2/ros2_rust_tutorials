...
static OBJECT_X: AtomicUsize = AtomicUsize::new(0);
static OBJECT_Y: AtomicUsize = AtomicUsize::new(0);
...
fn object_detect(image: &mut Mat) -> Result<()> {
    ...
    for (i, cnt) in contours.iter().enumerate() {
        ...
        OBJECT_X.store(center_x as usize, std::sync::atomic::Ordering::Relaxed);
        OBJECT_Y.store(center_y as usize, std::sync::atomic::Ordering::Relaxed);
...
fn main() -> Result<()> {
    ...
    let node = executor.create_node("object_server")?;
    let logger = node.logger().clone();
    log_info!(&logger, "ROS2节点示例：订阅节点");

    let logger_clone = logger.clone();

    let _server = node.create_service::<GetObjectPosition, _>(
        "get_object_position",
        move |request: GetObjectPosition_Request| -> GetObjectPosition_Response {
            log_info!(&logger_clone, "Incoming request: get={}", request.get);
            GetObjectPosition_Response { x: OBJECT_X.load(std::sync::atomic::Ordering::Relaxed) as i32, y: OBJECT_Y.load(std::sync::atomic::Ordering::Relaxed) as i32 }
        },
    )?;
